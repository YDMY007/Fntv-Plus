...
/* ══════════════════════════════════════════════════════════
   LIQUID GLASS 增强层 (iOS 26 风格液态玻璃)
   叠加在 ACRYLIC_CSS 之上: 在原有毛玻璃基础上增加
   「镜面高光边 / 湿润边缘 / 内阴影景深 / 窗口环境高光」,
   让 UI 元素看起来像一块块悬浮的液态玻璃。
   纯 CSS, 不破坏既有白底清除 / 弹窗 / 布局规则(①~⑭)。
   可逆: 在 injectAcrylicCSS 中注释掉 insertCSS(LIQUID_GLASS_CSS) 即退回亚克力。
   注: 真·折射(backdrop 形变)见文末「可选折射」说明, 默认不开启以免扰动 fnOS 排版。
   ══════════════════════════════════════════════════════════ */
const LIQUID_GLASS_CSS = `
    ...
    body{
        ...
        box-shadow:
            ...
            inset 0 0 60px rgba(255,255,255,.04)!important;
    }
    ...
    .fixed.inset-0[class*="lg:!hidden"] > *:not(.absolute){
        ...
        box-shadow:
            ...
            inset 0 -1px 0 rgba(120,110,140,.22),
            -10px 0 40px rgba(140,130,160,.12)!important;
    }
    .fixed.inset-0[class*="lg:!hidden"] > *:not(.absolute)::before{
        content:''!important;
        position:absolute!important;
        inset:0!important;
        border-radius:inherit!important;
        pointer-events:none!important;
        background:linear-gradient(155deg, rgba(255,255,255,.28) 0%, rgba(255,255,255,0) 30%)!important;
        mix-blend-mode:screen!important;
    }

    /* ── ⑦ 顶部导航栏: 镜面高光条 (顶部受光, 底部淡淡阴影) ── */
    div.relative.z-20.flex.items-center.justify-between.px-11.py-5{
        border-bottom:1px solid rgba(255,255,255,.35)!important;
        box-shadow:
            inset 0 1px 0 rgba(255,255,255,.6),
            inset 0 -1px 0 rgba(120,110,140,.12),
            0 6px 22px rgba(120,110,140,.10)!important;
    }
    div.relative.z-20.flex.items-center.justify-between.px-11.py-5::after{
        content:''!important;
        position:absolute!important;
        left:0!important;right:0!important;top:0!important;height:42%!important;
        pointer-events:none!important;
        background:linear-gradient(180deg, rgba(255,255,255,.30) 0%, rgba(255,255,255,0) 100%)!important;
    ...
    li[class*="cursor-pointer"]{
        ...
        box-shadow:
            0 2px 10px rgba(120,110,140,.10),
            0 1px 0 rgba(255,255,255,.6),
            inset 0 1px 0 rgba(255,255,255,.7),
            inset 0 -1px 0 rgba(120,110,140,.16),
            inset 0 0 14px rgba(255,255,255,.10)!important;
    }
    .card-root::after,
    [class*="card"]:not([class*="drawer"])::after,
    li[class*="cursor-pointer"]::after{
        content:''!important;
        position:absolute!important;
        inset:0!important;
        border-radius:inherit!important;
        pointer-events:none!important;
        background:linear-gradient(160deg, rgba(255,255,255,.40) 0%, rgba(255,255,255,0) 42%)!important;
        mix-blend-mode:screen!important;
    }
    /* 卡片 hover: 高光增强, 模拟光标压过液态表面 */
    [class*="card"]:not([class*="drawer"]):hover,
    li[class*="cursor-pointer"]:hover{
        box-shadow:
            0 4px 16px rgba(120,110,140,.16),
            0 1px 0 rgba(255,255,255,.7),
            inset 0 1px 0 rgba(255,255,255,.85),
            inset 0 -1px 0 rgba(120,110,140,.18),
            inset 0 0 20px rgba(255,255,255,.18)!important;
    }

    /* ── 窗口级: 顶部一道环境高光, 强化玻璃罩感 (极淡, 不挡点击) ── */
    body::before{
        content:''!important;
        ...
        background:radial-gradient(120% 60% at 50% -10%, rgba(255,255,255,.08) 0%, rgba(255,255,255,0) 55%)!important;
    }
    ...
    html.dark li[class*="cursor-pointer"]{
        ...
        box-shadow:
            ...
            inset 0 0 14px rgba(255,255,255,.04)!important;
    }
    html.dark .fixed.inset-0[class*="lg:!hidden"] > *:not(.absolute){
        ...
        box-shadow:
            ...
            -10px 0 40px rgba(0,0,0,.35)!important;
    }
    html.dark .fixed.inset-0[class*="lg:!hidden"] > *:not(.absolute)::before{
        background:linear-gradient(155deg, rgba(255,255,255,.12) 0%, rgba(255,255,255,0) 30%)!important;
    }
    html.dark div.relative.z-20.flex.items-center.justify-between.px-11.py-5{
        ...
        box-shadow:
            ...
            0 6px 22px rgba(0,0,0,.30)!important;
    }
    html.dark div.relative.z-20.flex.items-center.justify-between.px-11.py-5::after{
        background:linear-gradient(180deg, rgba(255,255,255,.12) 0%, rgba(255,255,255,0) 100%)!important;
    }
`;
...
function injectAcrylicCSS(wc: Electron.WebContents): void {
    ...
    if (isLoginPath(url)) {
        ...
        // 主界面: 完整亚克力玻璃壳 + 液态玻璃增强层
        wc.insertCSS(ACRYLIC_CSS);
        wc.insertCSS(LIQUID_GLASS_CSS);
    }
}
