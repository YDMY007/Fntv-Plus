...
// [lc-1136] watchdog 自愈揭示钩子: 由 api.ts 注入其 onShowsReady(=injectCarousel),
...
`;
  ...
  // [lc-1136] 收尾动画在途时绝不碰 timer: completeCarouselProgress(details-ready)可能先于本函数触发
  //   (媒体库区块就绪晚于详情补完, 真机 13:01/13:07 两度复现)。旧代码此处无条件 clearInterval
  //   会把在途收尾动画中途杀掉 → revealOnce 永不执行(revealed 永假死锁)且 _completing 卡 true
  //   → 伪进度也不再启动 → 骨架永久 0%。跳过伪进度启动, 让在途收尾动画接管新进度条
  //   (S.carouselBarFill 已指向本骨架)推满 100% 并回调揭示, 骨架原位被真轮播替换。
  if (!_completing) {
    ...
    S.carouselProgressTimer = window.setInterval(() => {
      ...
      if (S.carouselProgressPct >= 99) {
        ...
        if (S.diagStuckTicks > 125 && !S.diagStuckLogged) {
          ...
          // [lc-1136] 自愈兜底: 数据已可渲染(有横版图)却卡死 → 直接揭示轮播, 不让用户永久盯骨架。
          //   经 api.ts 的 onShowsReady 钩子(=injectCarousel)触发注入; api.ts → progress.ts 单向,
          //   回调经模块级 setter 注入避免反向 import 成环。
          if (landCnt > 0 && S.apiShows.length > 0) {
            S.carouselRevealed = true;
            S.carouselInited = false;
            completeCarouselProgress(() => {
              try { _revealHook && _revealHook(); } catch (_) { /* ignore */ }
            }, 'watchdog-stuck-reveal');
          }
        }
      } else {
        S.diagStuckTicks = 0;
...
let _completing = false; // [lc-1136] 收尾动画在途标志: buildLoadingPlaceholder 据此跳过伪进度(防杀在途收尾 timer 死锁)
