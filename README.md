...
- **🧩 插件面板** — 把 MPV 内置脚本能力封装成 GUI 可控插件；内置「跳过片头片尾」开关（自动加载飞牛 / theintrodb 跳过数据，可显示跳过按钮或自动跳过）。使用与开发详见本地 `docs/插件开发与使用文档.md`。
...
- **智能跳过** — 可在 MPV 播放器界面设置，支持三种跳过片头片尾模式：章节检查、手动设置片头片尾、快捷键跳过固定时长；客户端设置面板「插件」分类也提供「自动跳过片头片尾」开关（自动加载飞牛 / theintrodb 数据，详见本地 `docs/插件开发与使用文档.md`）。
